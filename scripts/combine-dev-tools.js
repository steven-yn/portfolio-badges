...
// 결합할 뱃지 파일들 (순서대로: Webpack, Vite, Storybook, Vitest, Jest, PNPM, Bun)
const badgeFiles = [
  "23-webpack-8dd6f9.svg",
  "24-vite-646cff.svg",
  "20-storybook-ff4785.svg",
  "18-vitest-6e9f18.svg",
  "19-jest-c21325.svg",
  "21-pnpm-f69220.svg",
  "22-bun-000000.svg",
];
...
// 개발 도구 및 패키지 매니저 뱃지 결합
async function combineDevTools() {
  console.log(
    "Webpack + Vite + Storybook + Vitest + Jest + PNPM + Bun 뱃지 결합을 시작합니다...\n"
  );
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="Webpack Vite Storybook Vitest Jest PNPM Bun">
  <title>Webpack Vite Storybook Vitest Jest PNPM Bun</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "dev-tools.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log(
    "✅ Webpack + Vite + Storybook + Vitest + Jest + PNPM + Bun 뱃지 결합이 완료되었습니다!"
  );
...
// 실행
combineDevTools().catch(console.error);
