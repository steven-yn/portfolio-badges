...
// 결합할 뱃지 파일들 (순서대로)
const badgeFiles = ["15-styled-components-db7093.svg", "14-sass-cc6699.svg"];
...
// Styled Components + Sass 뱃지 결합
async function combineStylingTools() {
  console.log("Styled Components + Sass 뱃지 결합을 시작합니다...\n");
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="Styled Components Sass">
  <title>Styled Components Sass</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "styling-tools.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log("✅ Styled Components + Sass 뱃지 결합이 완료되었습니다!");
...
// 실행
combineStylingTools().catch(console.error);
