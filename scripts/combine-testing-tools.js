...
// Vitest + Jest + Storybook 뱃지 결합
async function combineTestingTools() {
  console.log("Vitest + Jest + Storybook 뱃지 결합을 시작합니다...\n");
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="Vitest Jest Storybook">
  <title>Vitest Jest Storybook</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "testing-tools.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log("✅ Vitest + Jest + Storybook 뱃지 결합이 완료되었습니다!");
...
// 실행
combineTestingTools().catch(console.error);
