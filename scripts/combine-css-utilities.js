...
// Tailwind CSS + Vanilla Extract 뱃지 결합
async function combineCssUtilities() {
  console.log("Tailwind CSS + Vanilla Extract 뱃지 결합을 시작합니다...\n");
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="Tailwind CSS Vanilla Extract">
  <title>Tailwind CSS Vanilla Extract</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "css-utilities.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log("✅ Tailwind CSS + Vanilla Extract 뱃지 결합이 완료되었습니다!");
...
// 실행
combineCssUtilities().catch(console.error);
