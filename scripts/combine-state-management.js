...
// 결합할 뱃지 파일들 (순서대로)
const badgeFiles = [
  "10-tanstack-query-ff4154.svg",
  "11-recoil-0067a3.svg",
  "12-zustand-d9411e.svg",
  "13-pinia-ffd859.svg",
];
...
// TanStack Query + Recoil + Zustand + Pinia 뱃지 결합
async function combineStateManagement() {
  console.log(
    "TanStack Query + Recoil + Zustand + Pinia 뱃지 결합을 시작합니다...\n"
  );
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="TanStack Query Recoil Zustand Pinia">
  <title>TanStack Query Recoil Zustand Pinia</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "state-management.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log(
    "✅ TanStack Query + Recoil + Zustand + Pinia 뱃지 결합이 완료되었습니다!"
  );
...
// 실행
combineStateManagement().catch(console.error);
