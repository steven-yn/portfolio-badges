...
// 결합할 뱃지 파일들 (순서대로)
const badgeFiles = [
  "25-github-actions-2088ff.svg",
  "26-cloudflare-f38020.svg",
  "27-gcp-4285f4.svg",
];
...
// GitHub Actions + Cloudflare + GCP 뱃지 결합
async function combineCloudDevops() {
  console.log("GitHub Actions + Cloudflare + GCP 뱃지 결합을 시작합니다...\n");
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="GitHub Actions Cloudflare GCP">
  <title>GitHub Actions Cloudflare GCP</title>`;
  ...
  // 결합된 SVG 파일 저장
  const outputPath = path.join(outputDir, "cloud-devops.svg");
  fs.writeFileSync(outputPath, combinedSvg);

  console.log(
    "✅ GitHub Actions + Cloudflare + GCP 뱃지 결합이 완료되었습니다!"
  );
...
// 실행
combineCloudDevops().catch(console.error);
