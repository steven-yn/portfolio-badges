...
// TailwindCSS + Styled Components + Sass + Vanilla Extract 뱃지 결합
async function combineCssStylingTools() {
  console.log(
    "TailwindCSS + Styled Components + Sass + Vanilla Extract 뱃지 결합을 시작합니다...\n"
  );
  ...
  // 통합 SVG 생성
  let combinedSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${maxHeight}" role="img" aria-label="TailwindCSS Styled Components Sass Vanilla Extract">
  <title>TailwindCSS Styled Components Sass Vanilla Extract</title>`;
  ...
  console.log(
    "✅ TailwindCSS + Styled Components + Sass + Vanilla Extract 뱃지 결합이 완료되었습니다!"
  );
  ...
  console.log("\n📍 각 뱃지 위치:");
  badges.forEach((badge, index) => {
    const techName = badge.filename
      .replace(/^\d+-/, "")
      .replace(/-[a-f0-9]+\.svg$/, "")
      .replace(/-/g, " ")
      .replace(/\b\w/g, (l) => l.toUpperCase());
    console.log(
      `   ${index + 1}. ${techName}: x=${badge.x}, width=${badge.width}px`
    );
  });
}

// 실행
combineCssStylingTools().catch(console.error);
